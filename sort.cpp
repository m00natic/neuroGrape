...
void sort_init(sort_t * sort, board_t * board, const attack_t * attack, int depth, int height, int trans_killer, int ThreadId) {
   ...
   for (i = 0; i < KillerNb; i++) sort->killer[i] = Killer[ThreadId][height][i];
   ...
   while (true) {
      ...
      while (sort->pos < LIST_SIZE(sort->list)) {
         ...
         if (false) {
            ...
            if (move == sort->trans_killer) continue;
            if (move_is_killer(move,sort->killer)) continue;
			if (!pseudo_is_legal(move,sort->board)) continue;
      ...
      if (false) {
		 ...
		 for (i = 0; i < KillerNb; i++) {
            if (sort->killer[i] != MoveNone) LIST_ADD(sort->list,sort->killer[i]);
		 }
...
static int move_value(int move, const board_t * board, int height, int trans_killer, int ThreadId) {
   ...
   } else if ((killer_nb = killer_number(move,height,ThreadId)) >= 0) { // killer
...
static int killer_number(int move, int height, int ThreadId) {
   for (int i = 0; i < KillerNb; i++) {
	   if (move == Killer[ThreadId][height][i]) return i;
   }
   return -1;
}
