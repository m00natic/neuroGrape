...
static bool LazyEval = true;
static bool KingSafety = false;
static int KingSafetyMargin = 1600;
static bool king_is_safe[ColourNb];

static const int PieceActivityWeight = 256;
static const int KingSafetyWeight = 256;
static const int PassedPawnWeight = 256;
...
static const int QueenMobOpening = 2;
static const int QueenMobEndgame = 4;
...
static const int ShelterOpening = 256;
...
static void check_logeval_size()
{
	if(LogFileWriter != NULL)
	{
		...
		if(size >= 2097152)	//	file has become bigger than 2mb
		{
		  //std::stringstream tempStr;
			...
			LogFileUser = LogFile + ctime(&rawtime) + ".log";
			//++fileCounter;
			LogFileUser = LogFileUser.erase(LogFileUser.find_last_not_of(" \t\n")+1);
   ...
   for (colour = 0; colour < ColourNb; colour++) {
      ...
      for (ptr = &board->piece[me][1]; (from=*ptr) != SquareNone; ptr++) { // HACK: no king
         ...
         case Knight64:
            ...
            mob = -KnightUnit;
            kmob[0] = 0; kmob[1] = 0; kmob[2] = 0; kmob[3] = 0;
			kmob[4] = 0; kmob[5] = 0; kmob[6] = 0; kmob[7] = 0;
               
			if (me == White) {
			   if (WEAK_SQUARE_BLACK(from-33))
                  kmob[0] = unit[board->square[from-33]];
               if (WEAK_SQUARE_BLACK(from-31))
                  kmob[1] = unit[board->square[from-31]];
               if (WEAK_SQUARE_BLACK(from-18))
                  kmob[2] = unit[board->square[from-18]];
               if (WEAK_SQUARE_BLACK(from-14))
                  kmob[3] = unit[board->square[from-14]];
               if (WEAK_SQUARE_BLACK(from+33))
                  kmob[4] = unit[board->square[from+33]];
               if (WEAK_SQUARE_BLACK(from+31))
                  kmob[5] = unit[board->square[from+31]];
               if (WEAK_SQUARE_BLACK(from+18))
                  kmob[6] = unit[board->square[from+18]];
			   if (WEAK_SQUARE_BLACK(from+14))
                  kmob[7] = unit[board->square[from+14]];
			} else {
               if (WEAK_SQUARE_WHITE(from-33))
                  kmob[0] = unit[board->square[from-33]];
               if (WEAK_SQUARE_WHITE(from-31))
                  kmob[1] = unit[board->square[from-31]];
               if (WEAK_SQUARE_WHITE(from-18))
                  kmob[2] = unit[board->square[from-18]];
               if (WEAK_SQUARE_WHITE(from-14))
                  kmob[3] = unit[board->square[from-14]];
               if (WEAK_SQUARE_WHITE(from+33))
                  kmob[4] = unit[board->square[from+33]];
               if (WEAK_SQUARE_WHITE(from+31))
